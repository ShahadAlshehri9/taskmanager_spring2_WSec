...
public class ManagerController {
    ...
    public ManagerController(ProjectService projectService, AdminService adminService) {
    ...
    public List<ProjectDTO> searchProjects(@RequestParam String keyword) {
        return projectService.searchAll(keyword).stream().map(ProjectDTO::from).toList();
    }
    ...
    public ResponseEntity<ProjectDTO> create(@RequestBody Project project, Principal principal) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ProjectDTO.from(created));
    }
    ...
    public ProjectDTO update(@PathVariable Long id, @RequestBody Project data, Principal principal) {
        return ProjectDTO.from(projectService.updateAsManager(id, data, principal.getName()));
    }
    ...
    public ProjectDTO assignLeader(@PathVariable Long id, @PathVariable String username, Principal principal) {
        return ProjectDTO.from(projectService.assignLeader(id, username, principal.getName()));
    }
    ...
    public ProjectDTO revokeLeader(@PathVariable Long id, Principal principal) {
        return ProjectDTO.from(projectService.revokeLeader(id, principal.getName()));
    }
}
